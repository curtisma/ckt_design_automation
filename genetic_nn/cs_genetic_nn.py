...
def train(session, dataset, labels, gen_num, max_iter=500, batch_size=32):
...
def optimize(population, toolbox, top, bot, stats=None, halloffame=None, verbose=None):
    ...
    with tf.Session(graph=graph) as session:
        for gen in range(1, ngen+1):
            ...
            dataset, labels = split_population(population)
...
def main():
    ...
    # plotting the stats after the optimization but the training process is visualized inside optimize method
